...
class InfixOp(AST):
	...
	def analyze2(infixOp, state, implicitType):
		def opErr(lType, rType):
			logError(state, infixOp.opSpan, 'invalid operand types for operator `{}` ({} and {})'
				.format(infixOp.op.value, lType, rType))
		...
		if lIndefinite and rIndefinite:
			...
			if canAccommodate(types.Int32, infixOp.l.value) and canAccommodate(types.Int32, infixOp.r.value):
				r = state.analyzeNode2(infixOp.r, types.Int32)
				l = state.analyzeNode2(infixOp.l, types.Int32)
			elif canAccommodate(types.Int64, infixOp.l.value) and canAccommodate(types.Int64, infixOp.r.value):
				r = state.analyzeNode2(infixOp.r, types.Int64)
				l = state.analyzeNode2(infixOp.l, types.Int64)
			else:
				r = state.analyzeNode2(infixOp.r, types.UInt64)
				l = state.analyzeNode2(infixOp.l, types.UInt64)
		elif rIndefinite and not infixOp.l.hasBlockValue:
			l = state.analyzeNode2(infixOp.l, implicitType)
			if l:
				if type(infixOp.r) == IntLit and l.type and l.type.isPtrType:
					r = state.analyzeNode2(infixOp.r, types.ISize if infixOp.r.value < 0 else types.USize)
				else:
					r = state.analyzeNode2(infixOp.r, l.type)
		elif lIndefinite or infixOp.l.hasBlockValue:
			r = state.analyzeNode2(infixOp.r, implicitType)
			if r:
				if type(infixOp.l) == IntLit and r.type and r.type.isPtrType:
					l = state.analyzeNode2(infixOp.l, types.ISize if infixOp.l.value < 0 else types.USize)
				else:
					l = state.analyzeNode2(infixOp.l, r.type)
		else:
			l = state.analyzeNode2(infixOp.l, implicitType)
			r = state.analyzeNode2(infixOp.r, l.type if l and l.type else implicitType)
